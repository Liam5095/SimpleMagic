    ...
    public static void clientTick(TickEvent.ClientTickEvent event) {
        ...
        if (player != null) {
            if (player.getMainHandItem().getItem() == ModItems.MAGIC_WAND.get()) {
                idleParticle(player.level(), player.position().add(new Vec3(0,2,0)));
            }
        }
    }

        public static void spawnFireParticale (Level level, Vec3 pos) {
        ...
        public static void idleParticle (Level level, Vec3 pos){
            ...
            WorldParticleBuilder.create(LodestoneParticleRegistry.SPARKLE_PARTICLE)
                    .setScaleData(GenericParticleData.create(0.1f, 0.5f).setEasing(Easing.CIRC_IN).build())
                    .setTransparencyData(GenericParticleData.create(0.75f, 0.25f).build())
                    .setColorData(ColorParticleData.create(startingColor, endingColor).setCoefficient(1.4f).setEasing(Easing.BOUNCE_IN_OUT).build())
                    //.setSpinData(SpinParticleData.create(0.2f, 0.4f).setSpinOffset((level.getGameTime() * 0.2f) % 6.28f).setEasing(Easing.QUARTIC_IN).build())
                    .setLifetime(5)
